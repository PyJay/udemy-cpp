...
Begin by defining the function prototypes for the functions `fahrenheit_to_celsius` and `fahrenheit_to_kelvin` which are both passed a double value and
...
You may use the <cmath> function round in order to round the return value. The formula to convert Fahrenheit to Celsius is (5.0/9.0)*(temperature - 32). 
...
You can find my solution by clicking on the solution.txt file on the left pane. But please make sure you give it a go yourself first, and only check the solution
